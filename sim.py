...
def set_wheel_speed(v, w=0):
    ...
    p.setJointMotorControlArray(robot,
                                wheels,
                                p.VELOCITY_CONTROL,
                                targetVelocities=to_wheel_speeds.dot(
                                    [v, w]).clip(-100 * MAX_SPEED,
                                                 100 * MAX_SPEED))
...
def fold():
    ...
    set_wheel_speed(p.readUserDebugParameter(speed_param),
...
for i in range(100000):
    ...
    if p.readUserDebugParameter(reset_button) > reset_button_count:
        ...
        pos, orn = p.getBasePositionAndOrientation(robot)
        p.resetDebugVisualizerCamera(camera_info[10], camera_info[8],
                                     camera_info[9], pos)
