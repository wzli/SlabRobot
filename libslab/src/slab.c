...
static void slab_gamepad_input_update(Slab* slab) {
    ...
    float sticks[4] = {0};
    for (int i = 0; i < 4; ++i) {
        if (ABS((&slab->gamepad.left_stick.x)[i]) >=
                slab->config.joystick_threshold) {
    ...
    float speed_scale =
            0.5f * slab->config.wheel_diameter * slab->config.max_wheel_speed;
    if (slab->state.balance_active) {
        speed_scale /= 4;
    }
    // map L-stick to velocity input (dpad overrides stick)
    float* velocity_input =
            buttons[X] == 0 && buttons[Y] == 0 ? sticks : buttons;
    slab->input.linear_velocity = velocity_input[Y] * speed_scale;
    slab->input.angular_velocity = velocity_input[X] * speed_scale /
                                   (0.5f * slab->config.wheel_distance);
    // map R-stick to velocity input (button overrides stick)
    float* legs_input =
            (buttons[RX] == 0 && buttons[RY] == 0 ? sticks : buttons) + 2;
    ...
    for (int i = 0, sign = -1; i < 2; ++i, sign *= -1) {
        ...
        if (!GET_BIT(slab->gamepad.buttons, GAMEPAD_BUTTON_R1)) {
            slab->input.body_incline -= 0.5f * leg_speed * legs_input[Y] *
                                        SGN(slab->state.body_incline);
            int dir = slab->state.balance_active ? (only_contact ? 2 : -1) : 1;
            slab->input.leg_positions[i] +=
                    leg_speed * (legs_input[X] * (1 - only_contact) +
                                        legs_input[Y] * sign * dir);
            slab->input.leg_positions[i] = CLAMP(slab->input.leg_positions[i],
                    slab->config.min_leg_position,
                    slab->config.max_leg_position);
        } else if (!only_contact) {
            if (SQR(legs_input[X]) > SQR(legs_input[Y])) {
                LOW_PASS_FILTER(slab->input.body_incline,
                        SGN(slab->input.body_incline) * M_PI_2,
                        leg_speed * ABS(legs_input[X]));
                LOW_PASS_FILTER(slab->input.leg_positions[i],
                        SGN(legs_input[X]) * M_PI,
                        leg_speed * ABS(legs_input[X]));
            } else {
                LOW_PASS_FILTER(slab->input.body_incline,
                        (SGN(slab->input.body_incline) +
                                sign * SGN(legs_input[Y])) *
                                M_PI_2,
                        0.5f * leg_speed * ABS(legs_input[Y]));
                LOW_PASS_FILTER(slab->input.leg_positions[i],
                        sign * SGN(legs_input[Y]) * M_PI_2,
...
static void slab_differential_drive_update(
        Slab* slab, float v_lin, float v_ang) {
    ...
    slab->motors[MOTOR_ID_FRONT_LEFT_WHEEL].input.velocity =
            (legs & 1) ? left_wheel_speed : 0;
    slab->motors[MOTOR_ID_FRONT_RIGHT_WHEEL].input.velocity =
            (legs & 1) ? right_wheel_speed : 0;
    slab->motors[MOTOR_ID_BACK_LEFT_WHEEL].input.velocity =
            (legs & 2) ? left_wheel_speed : 0;
    slab->motors[MOTOR_ID_BACK_RIGHT_WHEEL].input.velocity =
            (legs & 2) ? right_wheel_speed : 0;
...
static void slab_state_update(Slab* slab) {
    ...
    axis_remap((Vector3F*) &slab->state.orientation.qx,
            (Vector3F*) &slab->imu.orientation.qx, slab->config.imu_axis_remap);
    // calculate roll
    ...
    for (int i = 0; i < 4; ++i) {
        QUAT_TRANSFORM((float*) &slab->state.verticies[i],
                (float*) &slab->state.orientation,
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (GET_BIT(slab->state.ground_contacts, i)) {
            if (slab->state.verticies[i].z >
                    slab->config.ground_rise_threshold) {
                slab->state.ground_contacts &= ~(1 << i);
            }
        } else {
            if (slab->state.verticies[i].z <
                    slab->config.ground_fall_threshold) {
                slab->state.ground_contacts |= 1 << i;
            }
        }
    }
    ...
    for (int i = 0; i < 4; ++i) {
        if (GET_BIT(slab->state.ground_contacts, i / 2)) {
            ++ground_wheels;
            slab->state.linear_velocity +=
                    slab->motors[2 + i].estimate.velocity;
        }
    }
    if (ground_wheels) {
        slab->state.linear_velocity *=
                0.5f * slab->config.wheel_diameter / ground_wheels;
    }
    // turn on balance control when only one leg has ground contact
    bool balance_active = slab->state.ground_contacts == 1 ||
                          slab->state.ground_contacts == 2;
